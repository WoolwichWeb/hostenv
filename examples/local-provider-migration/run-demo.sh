...
# =============================================================================
# Hostenv Local Migration Demo - Comprehensive Educational Version
# =============================================================================
#
# PURPOSE:
...
# 4. Orchestrates deployments across nodes using comin (the node agent)
#
# DEMO OVERVIEW:
...
# 4. Each node runs comin, which polls provider-service for actions
...
# - comin: The node-side agent that executes deployments
...
# -----------------------------------------------------------------------------
# load_comin_node_token: Get authentication credentials for a node
# -----------------------------------------------------------------------------
...
load_comin_node_token() {
  local node="$1"
  sops -d --output-type json "$PROVIDER_DIR/secrets/provider.yaml" | jq -r --arg node "$node" '.comin_node_tokens[$node] // empty'
}
...
# 4. Serves per-node action APIs that comin polls
#
# CONFIGURATION:
...
# - comin: Configuration for the node agent integration
...
ensure_provider_service_running() {
  ...
  # Write the comin node tokens file
  {
    printf 'node-a: %s\n' "$NODE_A_AUTH_TOKEN"
...
# -----------------------------------------------------------------------------
# trigger_comin_refresh: Force a node to check for deployments
# -----------------------------------------------------------------------------
# Comin is the node-side agent. Normally it polls provider-service every few
# seconds. For this demo, we trigger it immediately so users don't wait.
#
# HOW IT WORKS:
# We SSH to the node and run the comin activation wrapper with a specific
# git SHA. This simulates what comin would do when it polls and finds a new
# deployment intent.
#
# WHY THIS MATTERS:
# In production, comin's polling interval means deployments take ~30 seconds
# to start. For the demo, we want immediate feedback.

trigger_comin_refresh() {
  local node_alias="$1"
  local deploy_sha="$2"

  ssh -F "$SSH_CONFIG" "root@${node_alias}" "bash -s -- ${deploy_sha@Q}" <<'EOF_REFRESH' >/dev/null
set -euo pipefail
deploy_sha="${1:-}"
...
if [[ -n "$config_path" && -r "$config_path" ]]; then
  wrapper_path="$(grep -Eo '/nix/store/[^[:space:]]*hostenv-comin-activate-wrapper[^[:space:]]*' "$config_path" | head -n1 || true)"
fi
test -n "$wrapper_path"
test -x "$wrapper_path"
COMIN_GIT_SHA="$deploy_sha" "$wrapper_path"
EOF_REFRESH
}
...
# - provider settings: Node placement, comin config, service endpoints
...
# 4. Generate comin authentication tokens
...
prepare_baseline_node_a() {
  ...
  run_provider_plan
  (cd "$PROVIDER_DIR" && nix run .#hostenv-provider -- comin-tokens --quiet)
  refresh_merged_provider_secrets
  commit_generated

  load_plan_metadata
  load_project_hash_from_plan
  NODE_A_AUTH_TOKEN="$(load_comin_node_token node-a)"; [[ -n "$NODE_A_AUTH_TOKEN" ]] || fail "Missing node-a token"
...
prepare_baseline_node_b() {
  ...
  load_plan_metadata
  load_project_hash_from_plan
  NODE_B_AUTH_TOKEN="$(load_comin_node_token node-b)"; [[ -n "$NODE_B_AUTH_TOKEN" ]] || fail "Missing node-b token"
...
# 3. Trigger comin refresh to apply the deployment
...
deploy_to_node_a() {
  ...
  trigger_comin_refresh "node-a.${HOSTENV_HOSTNAME}" "$(resolve_intent_commit_sha_for_job "$NODE_A_JOB_ID")"
...
migrate_to_node_b() {
  ...
  trigger_comin_refresh "node-a.${HOSTENV_HOSTNAME}" "$intent_sha"
  trigger_comin_refresh "node-b.${HOSTENV_HOSTNAME}" "$intent_sha"
