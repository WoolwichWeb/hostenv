...
`seed.sql.gz` is a compressed MySQL dump for the local provider-service-project demo.

It contains:
- A Drupal 11 standard install database.
- A `hostenv_demo_marker` row used by migration verification.

The later demo flow copies this file into the app project as `.hostenv/seed.sql.gz` and asks the user to import it with:
