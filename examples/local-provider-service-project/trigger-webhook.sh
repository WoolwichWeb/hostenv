...
This helper is shared by the local provider migration demo and the local
provider-service-project demo scaffold.
...
Examples:
  ./examples/local-provider-service-project/trigger-webhook.sh --workdir /tmp/hostenv-demo
  ./examples/local-provider-service-project/trigger-webhook.sh --workdir /tmp/hostenv-demo --commit-sha 1a2b3c4d
USAGE
}
