...
let
  ...
  wrapperScriptsList = lib.mapAttrs
    (name: backup:
      let
  ...
  options.services.restic.backups = lib.mkOption {
    ...
    type = lib.types.attrsOf (lib.types.submodule ({ name, ... }: {
      options = {
        passwordFile = lib.mkOption {
          ...
          '';
          example = ''
            "/home/" + cfg.user + "/.config/restic-password"
          '';
        };
        ...
        runCheck = lib.mkOption {
          type = lib.types.bool;
          default = (builtins.length cfg.backups.${name}.checkOpts > 0);
        ...
        wantsUnits = lib.mkOption {
          ...
          default = [ ];
          description = ''
            List of systemd dependencies for this backup. See
            {manpage}systemd.unit(5), under 'Wants', for details.
          '';
    ...
    example = {
      localbackup = {
        ...
        passwordFile = ''/home/''${cfg.user}/.config/restic-password'';
        initialize = true;
      };
      remotebackup = {
        paths = [ "/home" ];
        ...
        passwordFile = ''/home/''${cfg.user}/.config/restic-password'';
        extraOptions = [
          ''sftp.command='ssh backup@host -i /home/''${cfg.user}/.config/backup-private-key -s sftp' ''
        ];
        timerConfig = {
          OnCalendar = "00:05";
          RandomizedDelaySec = "5h";
  ...
  config =
    {
        ...
        lib.mapAttrs'
          (name: backup:
            ...
            lib.nameValuePair "restic-backups-${name}" ({
              ...
              wants = [ "network-online.target" ] ++ backup.wantsUnits;
              after = [ "network-online.target" ];
              serviceConfig = {
                Type = "oneshot";
                ExecStart = (lib.optionals doBackup [ "${resticCmd} backup ${lib.concatStringsSep " " (backup.extraBackupArgs ++ excludeFlags)} --files-from=\${RUNTIME_DIRECTORY}/includes" ])
                  ++ pruneCmd ++ checkCmd;
      ...
      systemd.timers =
        lib.mapAttrs'
          ...
          (lib.filterAttrs (_: backup: backup.timerConfig != null) cfg.backups);

      activate =
        let
          scripts = lib.mapAttrsToList
            (name: script: "ln -s \"${script}/bin\"/* \"$HOME/.local/bin\"")
            wrapperScriptsList;
        in
        ''
          ## Restic activation script.
          mkdir -p "$HOME/.local/bin"

          existing=$(ls "$HOME/.local/bin" | grep '^restic-')
          for f in $existing; do
            if [ -L "$HOME/.local/bin/$f" ]; then
              # The script gets here only if the file is a symlink with a name
              # starting 'restic-'.
              rm "$HOME/.local/bin/$f"
            fi
          done
