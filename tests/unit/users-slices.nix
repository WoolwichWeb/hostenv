...
let
  ...
  slice_defaults_applied =
    ...
    in
    asserts.assertTrue "users-slices-configured" ok "slice defaults not applied";
  ...
  slice_respects_custom_user =
    ...
    in
    asserts.assertTrue "users-slices-custom-user" ok "custom user slice/user mismatch";
