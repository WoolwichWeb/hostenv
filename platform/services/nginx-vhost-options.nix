# Env-local copy of nixpkgs' nginx vhost options, trimmed to avoid host-only
# features (notably ACME) so env modules do not import NixOS host modules.
{ lib, config, pkgs, ... }:
...
with lib;
{
  options = {
    ...
    locations = mkOption {
      type = types.attrsOf (
        types.submodule (
          import ./nginx-location-options.nix {
            inherit lib config;
          }
        )
      );
