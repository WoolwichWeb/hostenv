...
''
  _hostenv_spinner() {
    ...
    _hostenv_spinner_ready="$_hostenv_spinner_output.ready"
    _hostenv_spinner_command_pid=""
    _hostenv_spinner_multiline=0

    _hostenv_spinner_output_has_marker() {
      [ -n "$_hostenv_spinner_activation_marker" ] || return 1
      while IFS= read -r _hostenv_spinner_line || [ -n "$_hostenv_spinner_line" ]; do
        if [ "$_hostenv_spinner_line" = "$_hostenv_spinner_activation_marker" ]; then
          return 0
        fi
    ...
    _hostenv_spinner_clear() {
      [ -e "$_hostenv_spinner_ready" ] || return 0
    ...
    trap '
      ...
      _hostenv_spinner_clear
      rm -f "$_hostenv_spinner_output" "$_hostenv_spinner_ready"
    ' EXIT
    ...
    _hostenv_neko_compact_frame() {
      ...
      printf '\r\033[2K%s%s%s %s·%s %s' \
        ...
        "$_hostenv_neko_cyan" "$_hostenv_neko_reset" "$_hostenv_spinner_title" >&2
      : > "$_hostenv_spinner_ready"
    }
    ...
    if [ "$_hostenv_spinner_can_animate" -eq 1 ]; then
    ...
    if [ "$_hostenv_spinner_can_animate" -eq 1 ]; then
      while kill -0 "$_hostenv_spinner_command_pid" 2>/dev/null; do
        if [ "$_hostenv_spinner_multiline" -eq 0 ]; then
          _hostenv_neko_compact_frame
        elif _hostenv_spinner_output_has_marker; then
          _hostenv_neko_render_activate
        else
    ...
    if [ "$_hostenv_spinner_show_output" -eq 1 ] \
      || { [ "$_hostenv_spinner_status" -ne 0 ] && [ "$_hostenv_spinner_show_error" -eq 1 ]; }; then
      _hostenv_spinner_replay_output
    fi

    if [ "$_hostenv_spinner_status" -eq 0 ] \
      && [ "$_hostenv_spinner_finish" = "sleep" ] \
      && [ "$_hostenv_spinner_can_animate" -eq 1 ] \
      && [ "$_hostenv_spinner_multiline" -eq 1 ]; then
      _hostenv_neko_finish_sleep
    fi
