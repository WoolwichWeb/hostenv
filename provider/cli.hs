...
cfUpsertCname token zoneId name target = do
  ...
  case filter ((== "CNAME") . rType) existing of
    (c:_) -> do
      ...
      _ <- Sh.inproc "curl" ["-sS","-X","PUT","-H","Authorization: Bearer " <> token,"-H","Content-Type: application/json","--data", body, url] Sh.empty
      pure ()
    [] -> do
      let url = "https://api.cloudflare.com/client/v4/zones/" <> zoneId <> "/dns_records"
      let body = "{\"type\":\"CNAME\",\"name\":\"" <> name <> "\",\"content\":\"" <> target <> "\",\"proxied\":false}"
      _ <- Sh.inproc "curl" ["-sS","-X","POST","-H","Authorization: Bearer " <> token,"-H","Content-Type: application/json","--data", body, url] Sh.empty
      pure ()
          ...
          then pure planAcc
            ...
            when hasCF $
              case (cfTok, cfZone) of
                (Just t, Just z) -> cfUpsertCname (T.pack t) (T.pack z) vhName expectedHost
                _ -> pure ()
