...
3. Add NixOS node configs under `nodes/<node>/configuration.nix` (with `system.stateVersion`).
4. Run `nix run .#hostenv-provider-plan` to write `generated/{flake.nix,plan.json,state.json}`.
5. Deploy using your tool of choice (e.g. deploy-rs) pointing at `generated/flake.nix`.
